class GamesController < ApplicationController
  ...
  def index
  @upcoming = Game.where("date >= ?", Date.today).order(:date)
  @past     = Game.where("date < ?",  Date.today).order(date: :desc)
  end
  ...
  private
    ...
    def game_params
      params.require(:game).permit(:date, :opponent, :place, :start_time, :score, :note)
    end
end
